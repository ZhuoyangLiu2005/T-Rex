...
class ViveStreamer(BaseStreamer):
    def __init__(self, vive_names: list[str], ip, port=5555, fps=20, **kwargs):
        self.vive_names = vive_names
    ...
    def get(self):
        ...
        # Initialize Vive data (vive_keys) - Vive only provides pose data
        vive_data = {}
        ...
        try:
            ...
            if combined_data:
                for vive_name in self.vive_names:
                    if vive_name in combined_data and combined_data[vive_name] is not None:
                        vive_data[vive_name] = get_transformation(combined_data[vive_name])

        except zmq.ZMQError as e:
            print(f"ZMQ Error while requesting Vive data: {e}")

        # Return structured output - Vive only provides pose data
