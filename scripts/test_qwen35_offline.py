"""
Offline test + ZeroMQ inference server for the Qwen3.5-based MoT VLA model.

Mirrors the logic of test_sharpa_tactile.py (Janus version) but adapted for:
  - Qwen35VLAModel  (modeling_vla_35.py)
  - AutoProcessor   (Qwen3.5 processor)
...
def model_load(args):
    """
    Returns (model, processor, statistic).
    ...
    # Determine the pretrained model path for architecture construction.
    # Priority: --base_model_path > checkpoint config.json > training_args.json
    ...
    if getattr(args, "base_model_path", "") and os.path.isdir(args.base_model_path):
        ...
        # Try to read base model path from training_args.json saved during training
    ...
    # Ensure all parameters are in bfloat16 (some modules like deform_proj may
    # stay float32 if their weights were not in model.pt or were freshly inited)
    model = model.to(torch.bfloat16)

    # ── 4. Normalisation statistics ───────────────────────────────────────────
    stats_path = args.stats_path or ""
    if not stats_path:
        # Try checkpoint dir first, then alongside the test JSON
        for candidate in [
...
def model_predict(
    ...
    """
    Run one denoising flow and return a list of action arrays (shape: action_chunk × action_dim).
    """
    ...
    with torch.inference_mode():
        ...
        if args.use_tactile_deform:
            # tactile_input: list of float arrays (H,W) in [0,1]  OR np.ndarray (N,H,W)
            if isinstance(tactile_input, (list, tuple)):
                arr = np.stack([
                    (np.array(t, dtype=np.float32) / 255.0
                     if t.dtype == np.uint8 else np.array(t, dtype=np.float32))
                    for t in tactile_input
                ])                                      # (N, H, W)
            else:
                arr = np.array(tactile_input, dtype=np.float32)
            # shape expected by forward_flow: [B, N, 1, H, W]
...
def main(args):
    ...
    if args.use_tactile_deform:
        dummy_tac = np.zeros((5, 240, 240), dtype=np.float32)   # (N, H, W)
    else:
        ...
        for step, sample in enumerate(it):
            ...
            try:
                ...
                if args.use_tactile_deform:
                    ...
                    tactile_input = np.array(sample["tactile_f6"], dtype=np.float32)
                ...
                predicted_action = model_predict(
                    args, model, processor, statistic, action_tokenizer,
                    task_description, slow_images, fast_images, tactile_input, state_fast,
                )
    ...
    print(f"VLA server (Qwen3.5) listening on port {args.port}...")
    ...
    while True:
        try:
            ...
            fast_image_list = [fast_wrist_r, fast_wrist_l]

            task_description = payload["task_description"]
            state_fast       = payload.get("state_fast", None)

            if args.use_tactile_deform:
                deform_bytes = payload.get("tactile_image_deform", [])
                tactile_input = [
                    np.array(Image.open(io.BytesIO(b)).convert("L"), dtype=np.float32) / 255.0
                    for b in deform_bytes
                ]
            else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Offline test + ZMQ server for Qwen3.5-based MoT VLA"
    )
