...
CFG (Classifier-Free Guidance) inference variant:
  When --tactile_cfg_scale != 1.0 and tactile inputs are present,
  runs two forward_flow passes (unconditional + conditional) and
  combines them: v_guided = v_uncond + cfg_scale * (v_cond - v_uncond).

Identical to test_qwen3vl_flare_offline.py except for the CFG logic above.
"""
...
def model_predict(
    ...
    with torch.inference_mode():
        ...
        # ── Flow-matching denoising (with optional CFG) ─────────────────
        noise = torch.randn(1, args.action_chunk, args.action_dim,
                            dtype=torch.bfloat16, device=device)

        cfg_scale = getattr(args, "tactile_cfg_scale", 1.0)
        has_tactile = (tac_f6_tensor is not None or tac_deform_tensor is not None)

        if has_tactile and cfg_scale != 1.0:
            # CFG: run unconditional (no tactile) and conditional (with tactile)
            # then combine: v_guided = v_uncond + cfg_scale * (v_cond - v_uncond)
            samples_uncond = model.forward_flow(
            ...
            samples_cond = model.forward_flow(
                inputs_embeds  = slow_embeds,
                position_ids   = position_ids,
                attention_mask = attention_mask,
                noise          = noise,
            ...
            # Guided combination
            samples = samples_uncond + cfg_scale * (samples_cond - samples_uncond)
        else:
            # Standard inference (no CFG needed)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Offline test + ZMQ server (with flare alignment + tactile CFG)")
    ...
    parser.add_argument("--tactile_cfg_scale", type=float, default=1.0,
                        help="Tactile guidance scale (1.0=standard, >1 amplifies tactile)")
