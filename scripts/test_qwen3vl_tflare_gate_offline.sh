...
python test_qwen3vl_tflare_gate_offline.py \
  ...
  --save_dir ./test_output_tflare_gate \
  --num_test_samples 300 \
  --port 5678 \
  --image_size 384 288

# Saved outputs (under save_dir):
#   action_dim_{i}.png          per-dimension pred vs GT
#   action_trajectory.npz
#   gate_vs_contact.png         scatter: mean gate vs max F6 magnitude
#   vtac_vact_diff_vs_contact.png   scatter: ‖v_tac − v_act‖ vs F6 magnitude
#   gate_per_action_dim.png     bar: which action dims the gate opens for
#   gate_diagnostics.npz
#   tflare_sim_per_step.png     bar: cos_sim per future tactile step
