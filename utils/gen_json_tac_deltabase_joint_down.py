...
def process_single_episode(episode_dir, save_dir, task_name, f_jsonl):
    ...
    for i in range(0, min_len):
        ...
        for k in range(ACTION_CHUNK):
            ...
            # Target action
            target_joint = original_absolute_actions[actual_future_idx]
            
            # Delta 动作：直接由目标帧减去 Chunk 的基准帧
            delta_joint = target_joint - chunk_base_state
            
            action_chunk_list.append(delta_joint.tolist())
            
        current_state = states[i].tolist()
        current_tactile_f6 = tactile_f6[i].tolist() 
        
        # Absolute target action for tracking error usage
        target_idx_next = min(i + FRAME_STRIDE - 1, min_len - 1)
        current_abs_action = original_absolute_actions[target_idx_next].tolist()
...
if __name__ == "__main__":
    ...
    JSON_NAME_BASE = "pick_orange_cube_0303_deltabase_clip_right_stride1_train"
