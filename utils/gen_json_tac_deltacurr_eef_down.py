...
def process_single_episode(episode_dir, save_dir, task_name, f_jsonl):
    ...
    strided_relative_actions = []
    strided_absolute_target = []
    
    for i in range(min_len):
        target_idx = min(i + FRAME_STRIDE - 1, min_len - 1)

        curr_state_arm_pose = s_r_arm_pose[i]

        targ_arm_pose = a_r_arm_pose[target_idx]
        targ_hand = a_r_hnd[target_idx]

        delta_9d = compute_chunk_delta_pose(curr_state_arm_pose, targ_arm_pose)
        
        rel_act = np.concatenate([delta_9d, targ_hand]).tolist()
        strided_relative_actions.append(rel_act)
        
        abs_act = absolute_targets[target_idx].tolist()
        strided_absolute_target.append(abs_act)
...
if __name__ == "__main__":
    ...
    JSON_NAME_BASE = "pick_orange_cube_0303_deltacurr_axis_eef_clip_right_stride2_train"
