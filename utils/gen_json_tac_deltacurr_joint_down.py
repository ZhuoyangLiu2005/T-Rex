...
DEFAULT_INSTRUCTION = "Turn a page of the book from right to left using your right index finger."
...
def process_single_episode(episode_dir, save_dir, task_name, f_jsonl):
    ...
    with h5py.File(h5_path, 'r') as f:
        s_r_arm = f['right_arm_joint_positions'][:]
        s_r_hnd = f['right_hand_joint_positions'][:]
        states = np.concatenate([s_r_arm, s_r_hnd], axis=1)
        
        a_r_arm = f['right_arm_target_dofs'][:]
        a_r_hnd = f['right_hand_target_joint_positions'][:]
        original_absolute_actions = np.concatenate([a_r_arm, a_r_hnd], axis=1)
    ...
    strided_relative_actions = np.zeros_like(states)
    strided_absolute_target = np.zeros_like(original_absolute_actions)
    
    for i in range(min_len):
        target_idx = min(i + FRAME_STRIDE - 1, min_len - 1)
        strided_relative_actions[i] = original_absolute_actions[target_idx] - states[i]
        strided_absolute_target[i] = original_absolute_actions[target_idx]
    ...
    for i in range(0, min_len):
        ...
        for k in range(ACTION_CHUNK):
            future_idx = min(i + k * FRAME_STRIDE, min_len - 1)
            act = strided_relative_actions[future_idx].tolist()
            action_chunk_list.append(act)
            
        current_state = states[i].tolist()
        current_tactile_f6 = tactile_f6[i].tolist() 
        current_abs_action = strided_absolute_target[i].tolist()
...
if __name__ == "__main__":
    DATA_ROOT = "/mnt/amlfs-02/shared/human_egocentric/dniu/Dex-MoT/mot_arch/data/bkl_inlab/20260313_flip_book_page_100"
    ...
    TASK_NAME = "flip_book_page_0313_clip_right_stride2"
    JSON_NAME_BASE = "flip_book_page_0313_deltacurr_joint_clip_right_stride2_train"
